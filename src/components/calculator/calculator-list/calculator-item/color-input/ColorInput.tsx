...
const ColorInput: FC<{ id: number }> = ({ id }) => {
    ...
    const [chart_color, setChartColor] = useState(calculation_data.data[id].color)
    useEffect(() => {
        color_count += 1
        ...
        setTimeout(() => {
            if (copy_count === color_count) {
                color_count = 0
                calculation_data.data[id].color = chart_color
                setCalculationData(calculation_data)
            }
        }, 500)
    }, [chart_color])
    useEffect(() => {
        setChartColor(calculation_data.data[id].color)
    }, [calculation_data])
